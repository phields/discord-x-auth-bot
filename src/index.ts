...
app.get("/oauth/x/callback", async (c) => {
  ...
  try {
    const result = await completeXOAuth(c.env, state, code);
    if (result.guildId !== c.env.DISCORD_GUILD_ID) return c.text("服务器不匹配。", 400);
    const roleResponse = await fetch(
      `https://discord.com/api/v10/guilds/${result.guildId}/members/${result.discordUserId}/roles/${c.env.DISCORD_VERIFIED}`,
      {
        method: "PUT",
        headers: {
    ...
    if (!roleResponse.ok) {
      ...
      console.error(JSON.stringify({
        ...
        roleId: c.env.DISCORD_VERIFIED,
        responseBody,
      }));
      return c.text("X 验证成功，但身份组分配失败，请联系管理员。", 502);
    }
    const removeResponse = await fetch(
      `https://discord.com/api/v10/guilds/${result.guildId}/members/${result.discordUserId}/roles/${c.env.DISCORD_UNVERIFIED}`,
      {
        method: "DELETE",
        headers: {
          authorization: `Bot ${c.env.DISCORD_TOKEN}`,
